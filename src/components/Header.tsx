...
// ─── README GENERATOR ────────────────────────────────────────────────────────────────────────────────────
// SPRINT-C-FIX-18: Generates a README.md written into the ZIP root.
// Pure function — no side effects. Gives first-time users everything they
// need to run the project immediately after unzipping.
// dataSources: used to surface required env var names (values never logged).
const buildReadme = (
    framework: string,
    projectDisplayName: string,
    pages: Array<{ name: string; slug: string }>,
    dataSources: Array<{ name: string; envVarMap?: Record<string, string> }>,
): string => {
    const isNext = framework === 'nextjs';
    const fwLabel    = isNext ? 'Next.js 14 (App Router)' : 'Vite + React (SPA)';
    const devCmd     = 'npm run dev';
    const buildCmd   = isNext ? 'npm run build && npm start' : 'npm run build';
    const previewCmd = isNext ? '' : '\nnpm run preview   # preview production build';
    const now = new Date().toISOString().split('T')[0];

    // Collect all env var keys that need to be set
    const envVarNames = new Set<string>();
    dataSources.forEach(ds => {
        if (ds.envVarMap) Object.keys(ds.envVarMap).forEach(k => envVarNames.add(k));
    });

    const envSection = envVarNames.size > 0
        ? [
            '',
            ...
            '',
            '```',
            ...[...envVarNames].map(k => `${k}=your_value_here`),
            '```',
            '',
    ...
    const pagesSection = pages.length > 0
        ? ['', '## Pages', '', ...pages.map(p => `- **${p.name}** \u2192 \`${p.slug}\``)].join('\n')
        : '';

    const deploySection = isNext
        ? ['', '## Deploy', '', '**Vercel (recommended)**', '```bash', 'npx vercel', '```', '', 'Or connect your GitHub repo at [vercel.com](https://vercel.com).'].join('\n')
        : ['', '## Deploy', '', '**Netlify / Vercel static**', '```bash', 'npm run build', '# Upload the `dist/` folder to any static host', '```'].join('\n');
    ...
    return [
        `# ${projectDisplayName}`,
        '',
        `Generated by [Vectra Visual Builder](https://vectra.dev) on ${now}.`,
        `Framework: **${fwLabel}**`,
        '',
        '## Quick Start',
        '',
        ...
        '---',
        '',
        '_This project was visually designed in Vectra and exported as production-ready code._',
        '_Edit any file — there is no Vectra runtime dependency in this export._',
        '',
    ].join('\n');
};
export const Header = () => {
    ...
    const handleExportZip = async () => {
        ...
        try {
            ...
            // SPRINT-C-FIX-18: use zipFileName for the ZIP filename to avoid
            // shadowing the `projectName` from useEditor() context.
            const zipFileName = framework === 'nextjs' ? 'vectra-nextjs' : 'vectra-vite';
            console.log(`[Vectra] 📦 Building ${zipFileName} ZIP from VFS...`);
            await addDir('/', zip);
            ...
            console.log('[Vectra] 📄 README.md injected into ZIP');
