...
export const RightSidebar = () => {
    ...
    // AI-SOURCE-1: true when this element was generated by the AI pipeline and
    // carries provenance metadata. Drives the AI tab visibility + auto-switch.
    const isAiNode = !!(element?.aiSource);
    ...
    useEffect(() => {
        ...
        // AI-SOURCE-1: prefer the AI tab for aiSource nodes — more useful context than the Code tab.
        // Falls through to Code tab only if aiSource is absent (manually placed custom_code).
        if ((element as any).aiSource) { setActiveTab('ai'); return; }
    ...
    return (
        <div id="right-sidebar" className="w-[320px] bg-[#333333] border-l border-[#252526] h-full flex flex-col">
            ...
            <div className="flex border-b border-[#252526] bg-[#333333]">
                {/* AI tab — only for aiSource-stamped nodes */}
                {isAiNode && (
                    <TabButton
                        active={activeTab === 'ai'}
            ...
            <div className={cn(
                ...
                {activeTab === 'ai' && isAiNode && element && (() => {
                    const src = (element as any).aiSource!;
                    const age = Date.now() - src.generatedAt;
                    ...
                    const handleRefine = async (promptOverride?: string) => {
                        ...
                        setIsRefining(true);
                        try {
                            await runAI(`Edit ${src.sectionName}: ${p}`);
                        } finally {
                            setIsRefining(false);
                        ...
                        <div className="flex flex-col h-full overflow-y-auto custom-scrollbar">
                            ...
                            <div className="p-3 border-b border-[#252526]">
                                <div className="flex items-center gap-2 mb-2.5">
                                    ...
                                    <div>
                                        <p className="text-[11px] font-bold text-violet-300">AI Section</p>
                                        <p className="text-[9px] text-zinc-600">{src.sectionName}</p>
                                    </div>
