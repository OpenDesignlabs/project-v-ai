...
const toPascalCase = (raw: string): string => {
  // ENGINE v0.4: Rust to_pascal_case (§15)
...
const wrapWithImportsNext = (rawCode: string): string => {
  // ENGINE v0.4: Rust wrap_component_next (§15)
  const wasm = (window as any).vectraWasm;
  if (wasm?.wrap_component_next) {
    try { return wasm.wrap_component_next(rawCode) as string; } catch { /* fall through */ }
  }
  return `'use client';
...
const wrapWithImportsVite = (rawCode: string): string => {
  // ENGINE v0.4: Rust wrap_component_vite (§15)
  const wasm = (window as any).vectraWasm;
