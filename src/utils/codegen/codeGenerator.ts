...
const generateNodeCode = (nodeId: string, project: VectraProject, imports: ImportManager, depth: number): string => {
  ...
  if (node.props.href) {
    ...
    return `${indent}<a href="${String(node.props.href)}" target="${target}"${relAttr} className="contents">\n${code}${indent}</a>\n`;
  }
 ...
 * Next.js App Router loading UI (shown during Suspense boundary transitions).
...
const NEXT_ERROR_TSX = `'use client';
/**
 * app/error.tsx — Auto-generated by Vectra Visual Builder
...
export default function Error({
  ...
  useEffect(() => {
    console.error('[Vectra] Route error:', error);
  }, [error]);

  return (
    <div className="flex items-center justify-center min-h-screen bg-black px-4">
      <div className="flex flex-col items-center gap-6 text-center max-w-sm">
        <div className="w-12 h-12 rounded-full bg-red-500/10 border border-red-500/20 flex items-center justify-center text-red-400 text-xl select-none">
          ⚠
        </div>
        <div>
          ...
          {error.digest && (
            <p className="mt-2 text-zinc-700 text-xs font-mono">digest: {error.digest}</p>
          )}
        </div>
        <button
          onClick={reset}
          className="px-5 py-2.5 bg-white text-black text-sm font-bold rounded-lg hover:bg-zinc-100 active:scale-95 transition-all"
        >
          Try again
        </button>
      </div>
